...
axes[1].set_xlabel('Predicted', fontsize=22)
axes[1].set_ylabel('Actual', fontsize=22)
axes[1].tick_params(axis='both', which='major', labelsize=22)

sns.heatmap(cm_gb, annot=True, fmt='d', ax=axes[2], cmap='Blues',
            xticklabels=['Low', 'Medium', 'High'], yticklabels=['Low', 'Medium', 'High'],
            annot_kws={"size": 24})
axes[2].set_title('(c)', fontsize=22)
axes[2].set_xlabel('Predicted', fontsize=22)
axes[2].set_ylabel('Actual', fontsize=22)
axes[2].tick_params(axis='both', which='major', labelsize=22)

plt.tight_layout()
plt.savefig('confusion_matrices.png', dpi=600)
plt.show()
